...
# Date of creation: 01/24/22 (sorry! next time I will start earlier)
...
# ***Author note***: I'm fluent in MATLAB, and used to know Python really well, so I'm pushing myself to re-learn Python
# with this class. This is my first time in a long time writing a Python script, so I'm commenting some simple things
# just for my own knowledge to try to remember them. I also tried to comment out anything that wasn't relevant to
# actually solving the homework problem. Sorry if it seems like a lot!
########################################################################################################################
...
# The fourier transform shifts from time domain to freq domain so that you can pick out certain frequencies
...
# find the maximum value using np.partition to get the max and second max
...
kxmax1 = K_grid[fft_max_location1[1]]  # x and y indices are swapped
kymax1 = K_grid[fft_max_location1[0]]
kzmax1 = K_grid[fft_max_location1[2]]

kxmax2 = K_grid[fft_max_location2[1]]  # x and y indices are swapped
...
# 1st max - FFT Average
plt.figure(3)
im2 = plt.contourf(xf[:, :, 10], yf[:, :, 10], fft_average[:, :, 10])  # not sure if this should be 0 to 64 or -10 to 10
plt.colorbar(im2)
plt.xlabel('x')
plt.ylabel('y')
...
# plot isosurface of FFT Shifted
for j in range(0, 49, 1):
...
# FILTER - keep waves that sub is emitting
...
sigma = 2  # kill stuff outside range
g_vals = filter(kx, ky, kz, sigma)  # determine filter # frequency domain bc will be applied to fft
...
# plot isosurface of filter itself
for j in range(0, 49, 1):
...
# plot the filter in slices to see whats up
plt.figure(5)
...
# plot isosurface of the filtered fft
for j in range(0, 49, 1):
